...
namespace EDSDKLib
{
    ...
    public class SDKHandler : IDisposable
    {
        ...
        public event EventHandler ImageSavedToDirectory;
        ...
        private uint Camera_SDKObjectEvent(uint inEvent, IntPtr inRef, IntPtr inContext)
        {
            ...
            switch (inEvent)
            {
                ...
                case EDSDK.ObjectEvent_DirItemCreated:
                    //OnObjectEventDirItemCreated(inRef, inContext);
                    if (DownloadVideo) { DownloadImageToDirectory(inRef, ImageSaveDirectory); DownloadVideo = false; }
                    break;
                ...
                case EDSDK.ObjectEvent_DirItemRequestTransfer:
                    ...
                    //DownloadImageToMemory(inRef);
                    //DownloadImageToDirectory(inRef, ImageSaveDirectory);
                    DownloadImage(inRef);
                    break;
        ...
        public void DownloadImageToDirectory(IntPtr ObjectPointer, string directory)
        {
            ...
            SendSDKCommand(delegate
            {
                ...
                {
                    string extension = Path.GetExtension(CurrentPhoto).ToLower();
                    if (!string.IsNullOrEmpty(extension) && (extension == ".jpg" || extension == ".jpeg" ))
                    {
                        //Fire Image saved to directory
                        if (ImageSavedToDirectory != null) ImageSavedToDirectory(null, null);
                    }
                }
        ...
        /// <param name="ObjectPointer">Pointer to the object. Get it from the SDKObjectEvent.</param>
        /// <param name="directory">Path to where the image will be saved to</param>
        public byte[] DownloadImageToMemory(IntPtr ObjectPointer)
        {
            byte[] resBytesArray = null;

            EDSDK.EdsDirectoryItemInfo dirInfo;
            IntPtr memoryStreamRef;
            //get information about object
            Error = EDSDK.EdsGetDirectoryItemInfo(ObjectPointer, out dirInfo);
            
            SendSDKCommand(delegate
            {
                Error = EDSDK.EdsCreateMemoryStream(dirInfo.Size, out memoryStreamRef);
        ...
        private byte[] ConvertImageStreamToBytes(IntPtr imageStream)
        {
            uint Error;
            IntPtr imagePtr;
